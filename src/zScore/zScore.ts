 ...
 */
export default function zScore(x: number, values: number[]): number {
  const n = values.length;
  if (n < 2) throw new Error("At least two values are required to compute z-score");

  const mean = values.reduce((sum, v) => sum + v, 0) / n;
  const variance = values.reduce((sum, v) => sum + (v - mean) ** 2, 0) / (n - 1);
  const stdDev = Math.sqrt(variance);

  if (stdDev === 0) throw new Error("Standard deviation is zero, cannot compute z-score");
